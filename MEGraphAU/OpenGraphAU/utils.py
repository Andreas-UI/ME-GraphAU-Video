...
def draw_text(img, words, probs, point):
    import cv2
    ...
    pt1, pt2 = point
    # emotions = {
    #     "Happy": round(float(probs[4]), 2) + round(float(probs[9]), 2),
    #     "Sad": round(float(probs[0]), 2) + round(float(probs[2]), 2) + round(float(probs[12]), 2),
    #     "Surprise": round(float(probs[0]), 2) + round(float(probs[1]), 2) + round(float(probs[3]), 2) + round(float(probs[22]), 2),
    #     "Fear": round(float(probs[0]), 2) + round(float(probs[1]), 2) + round(float(probs[2]), 2) + round(float(probs[3]), 2) + round(float(probs[5]), 2) + round(float(probs[17]), 2) + round(float(probs[23]), 2),
    #     "Anger": round(float(probs[2]), 2) + round(float(probs[3]), 2) + round(float(probs[5]), 2) + round(float(probs[19]), 2),
    #     "Disgust": round(float(probs[6]), 2) + round(float(probs[12]), 2) + round(float(probs[13]), 2) ,
    #     "Contempt": round(float(probs[38]), 2) + round(float(probs[40]), 2)
    # }
    thrshold = 0.15
    emotions = {
        "Happy": (float(probs[4]) > thrshold) and (float(probs[9]) > thrshold),
        "Sad": (float(probs[0]) > thrshold) and (float(probs[2]) > thrshold) and (float(probs[12]) > thrshold),
        "Surprise": (float(probs[0]) > thrshold) and (float(probs[1]) > thrshold) and (float(probs[3]) > thrshold) and (float(probs[22]) > thrshold),
        "Fear": (float(probs[0]) > thrshold) and (float(probs[1]) > thrshold) and (float(probs[2]) > thrshold) and (float(probs[3]) > thrshold) and (float(probs[5]) > thrshold) and (float(probs[17]) > thrshold) and (float(probs[23]) > thrshold),
        "Anger": (float(probs[2]) > thrshold) and (float(probs[3]) > thrshold) and (float(probs[5]) > thrshold) and (float(probs[19]) > thrshold),
        "Disgust": (float(probs[6]) > thrshold) and (float(probs[12]) > thrshold) and (float(probs[13]) > thrshold),
        "Contempt": (float(probs[38]) > thrshold) and (float(probs[40]) > thrshold)
    }
    emotions = [emotion for emotion, is_true in emotions.items() if is_true]
    emotion = "-".join(emotions)
    img = cv2.putText(img, str(emotion), (pt1[0],
                      pt2[1] + 20), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2)
    ...
    img = cv2.copyMakeBorder(
        img, 0, 0, 0, img.shape[1], cv2.BORDER_CONSTANT, value=(255, 255, 255))
    # num_aus = len(words)
    ...
    for i, item in enumerate(range(21)):
        ...
        if float(probs[item]) > 0.5:
            color = (0, 0, 255)
        img = cv2.putText(img,  AU_names[i] + ' -- AU' + AU_ids[i] + ': {:.2f}'.format(
            probs[i]), (pos_x, y), cv2.FONT_HERSHEY_SIMPLEX, round(img.shape[1] / 2800, 3), color, 2)
    ...
    for i, item in enumerate(range(21, 41)):
        ...
        if float(probs[item]) > 0.5:
            color = (0, 0, 255)
        img = cv2.putText(img,  AU_names[item] + ' -- AU' + AU_ids[item] + ': {:.2f}'.format(
            probs[item]), (pos_x_, y), cv2.FONT_HERSHEY_SIMPLEX, round(img.shape[1] / 2800, 3), color, 2)

    for i, item in enumerate(range(41)):
        au = AU_names[i] + ' -- AU' + AU_ids[i]
